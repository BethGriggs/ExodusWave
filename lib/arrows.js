    ...
    mouseMoveHandler.setInputAction(function (movement) {
        ...
        if (Cesium.defined(pickedObject)) {
            ...
            if (dataJson.data["1960"].hasOwnProperty(objectName)) {
                ...
                document.getElementById("infoDisplay-from").textContent = dataHandler.data.ref[countries[0]].short;
                document.getElementById("infoDisplay-to").textContent = dataHandler.data.ref[countries[1]].short;
                var startYear = parseInt(yearSlider.noUiSlider.get()[0]);
                var endYear = parseInt(yearSlider.noUiSlider.get()[1]);
                if (startYear == endYear) {
                    document.getElementById("infoDisplay-value").textContent =
                        startYear + ": " +
                        dataHandler.data.data[startYear][countries[0]].to[countries[1]];
                } else {
                    document.getElementById("infoDisplay-value").innerHTML =
                        startYear + ": " +
                        dataHandler.data.data[startYear][countries[0]].to[countries[1]]
